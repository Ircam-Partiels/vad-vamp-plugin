<h1 align="center">VAD Vamp Plugin Manual</h1>

<p align="center">
<i>Version APPVERSION for Windows, Mac & Linux</i><br>
<i>Manual by Pierre Guillot</i><br>
<a href="www.ircam.fr">www.ircam.fr</a><br><br>
</p>

## Table of contents

1. [Introduction](#introduction)
2. [Requirements](#system-requirements)
3. [Installation](#installation)
4. [Credits](#credits)

## Introduction

The VAD Vamp plugin is an implementation of the [VAD](https://github.com/dpirch/libfvad) voice activity detection library developed by WebRTC project authors as a Vamp plugin as a [Vamp plugin](https://www.vamp-plugins.org/).

The VAD Vamp Plugin has been designed for use in the free audio analysis application [Partiels](https://forum.ircam.fr/projects/detail/partiels/).

## Requirements

- Windows 10
- MacOS 13.3 (ARM)
- Linux

## Installation

Use the installer for your operating system. The plugin dynamic library (*vad.dylib* for MacOS, *vad.dll* for Windows and *vad.so* for Linux) and the category file (*vad.cat*) will be installed in your operating system's Vamp plugin installation directory:
- Linux: `~/vamp`
- MacOS: `/Library/Audio/Plug-Ins/Vamp`
- Windows: `C:\Program Files\Vamp`

## Credits

- **[libfvad](https://github.com/dpirch/libfvad)** Copyright (c) 2011, The WebRTC project authors. All rights reserved. Copyright (c) 2016 Daniel Pirch
- **[VAD Vamp plugin](https://www.ircam.fr/)** by Pierre Guillot at IRCAM IMR Department.   
- **[Vamp SDK](https://github.com/vamp-plugins/vamp-plugin-sdk)** by Chris Cannam, copyright (c) 2005-2024 Chris Cannam and Centre for Digital Music, Queen Mary, University of London.  
- **[Ircam Vamp Extension](https://github.com/Ircam-Partiels/ircam-vamp-extension)** by Pierre Guillot at [IRCAM IMR department](https://www.ircam.fr/).  

